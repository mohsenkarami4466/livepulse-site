...
function handleSmallGlobeClick(e) {
    ...
    const log = window.logger || { info: console.log }; log.info('🖱️ کلیک روی کره کوچک:', e.type);
    ...
    if (globeOpening) {
        const log = window.logger || { info: console.log }; log.info('⏳ کره در حال باز شدن است...');
        return;
    }
    
    globeOpening = true;
    const log = window.logger || { info: console.log }; log.info('🚀 در حال باز کردن کره بزرگ...');
    
    // باز کردن کره مالی
    if (typeof openFinancialGlobe === 'function') {
        openFinancialGlobe();
    } else {
        const log = window.logger || { error: console.error }; log.error('❌ تابع openFinancialGlobe یافت نشد!');
    }
...
function createAdvancedGlobe(containerId, type) {
    ...
    const ensureSize = () => {
        ...
        const log = window.logger || { info: console.log }; log.info(`🔍 بررسی اندازه container (تلاش ${retryCount + 1}/${maxRetries}):`, {
            width,
            height,
        ...
        if (width === 0 || height === 0) {
            ...
            if (retryCount >= maxRetries) {
                const log = window.logger || { error: console.error }; log.error('❌ Container بعد از 20 تلاش هنوز اندازه ندارد!');
                container.innerHTML = `
        ...
        const log = window.logger || { info: console.log }; log.info('✅ Container اندازه دارد، شروع ساخت کره...');
        createGlobe();
    };
    ...
    const createGlobe = () => {
        try {
            ...
            if (width === 0 || height === 0) {
                const log = window.logger || { warn: console.warn }; log.warn('Container هنوز اندازه ندارد، دوباره تلاش می‌کنم...');
                setTimeout(ensureSize, 100);
                return;
            }
            
            const log = window.logger || { info: console.log }; log.info(`🌍 ساخت کره ${type} با اندازه: ${width}x${height}`);
            ...
            const log = window.logger || { info: console.log }; log.info('✅ Renderer ساخته شد و به DOM اضافه شد');
            ...
            const log = window.logger || { info: console.log }; log.info('✅ کره با رنگ ساده ساخته شد');
            ...
            const tryLoadEarthTexture = (index) => {
                ...
                textureLoader.load(
                    ...
                    (texture) => {
                        const log = window.logger || { info: console.log }; log.info('✅ Texture لود شد:', EARTH_TEXTURE_PATHS.day[index]);
                        try {
                            // تنظیمات texture
                            ...
                            const log = window.logger || { info: console.log }; log.info('✅ Texture به کره اعمال شد');
                        } catch (texError) {
                            const log = window.logger || { warn: console.warn }; log.warn('⚠️ خطا در اعمال texture:', texError);
                        }
                    },
                    (progress) => {
                        if (progress && progress.total) {
                        const log = window.logger || { info: console.log }; log.info('📥 لود texture:', Math.round((progress.loaded / progress.total) * 100) + '%');
                        }
                    },
                    (error) => {
                        const log = window.logger || { warn: console.warn }; log.warn(`⚠️ Texture ${EARTH_TEXTURE_PATHS.day[index]} لود نشد، تلاش بعدی...`);
            ...
            const log = window.logger || { error: console.error }; log.error('❌ خطا در ساخت کره:', error);
            container.innerHTML = `
...
function setupScene(scene, camera, renderer, globe, type, container) {
    const log = window.logger || { info: console.log }; log.info('🔧 راه‌اندازی صحنه...');
    ...
    try {
        if (typeof THREE !== 'undefined' && typeof THREE.OrbitControls !== 'undefined') {
            ...
            const log = window.logger || { info: console.log }; log.info('✅ OrbitControls ساخته شد');
        } else {
            const log = window.logger || { warn: console.warn }; log.warn('⚠️ OrbitControls لود نشده است. کنترل‌ها غیرفعال هستند.');
        }
    } catch (error) {
        const log = window.logger || { error: console.error }; log.error('❌ خطا در ساخت OrbitControls:', error);
    }
    ...
    // اضافه کردن markers
    const log = window.logger || { info: console.log }; log.info('📍 اضافه کردن markers...');
    ...
    isAnimating = true;
    animate();
    const log = window.logger || { info: console.log }; log.info('✅ انیمیشن شروع شد');
    ...
    function handleResize() {
        ...
        if (width > 0 && height > 0) {
            ...
            renderer.setSize(width, height);
            const log = window.logger || { info: console.log }; log.info(`📐 ریزایز: ${width}x${height}`);
        }
    }
    ...
    const log = window.logger || { info: console.log }; log.info(`✅ کره ${type} کاملاً راه‌اندازی شد و آماده نمایش است!`);
    
    // تست رندر اولیه
    setTimeout(() => {
        try {
            renderer.render(scene, camera);
            const log = window.logger || { info: console.log }; log.info('✅ تست رندر اولیه موفق بود');
        } catch (error) {
            const log = window.logger || { error: console.error }; log.error('❌ خطا در تست رندر:', error);
        }
    }, 100);
}
...
function loadIranProvincialBorders(scene) {
    const log = window.logger || { info: console.log }; log.info('🗺️ بارگذاری مرزهای استانی و شهری ایران...');
    
    if (!scene || !scene.scene || !scene.earth) return;
    if (typeof iranProvinces === 'undefined') {
        const log = window.logger || { warn: console.warn }; log.warn('⚠️ داده‌های استان‌های ایران پیدا نشد');
        return;
    }
    ...
    const log = window.logger || { info: console.log }; log.info(`✅ مرزهای ${provinces.length} استان ایران اضافه شدند`);
}
...
function openPriceDetail(item) {
    const log = window.logger || { info: console.log }; log.info('🎯 مودال جدید فراخوانی شد برای:', item.name);
    ...
    const log = window.logger || { info: console.log }; log.info('✅ مودال با موفقیت باز شد');
}
/**
...
function setupAiChat() {
    ...
    if (aiChatBar && aiChatFab) {
        ...
        aiChatFab.onclick = function(e) {
            e.preventDefault();
            e.stopPropagation();
            const log = window.logger || { info: console.log }; log.info('💬 کلیک روی هدر چت');
            ...
            const log = window.logger || { info: console.log }; log.info('💬 صفحه فعلی:', currentPage);
